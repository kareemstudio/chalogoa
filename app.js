(function () {
  ...
  function renderCollectionChips() { const w = $("#radio-collections"); if (!w) return; w.innerHTML = ""; const all = document.createElement("button"); all.className = "collection-chip" + (state.collection === "all" ? " active" : ""); all.innerHTML = '<strong>All songs</strong><small>' + SONGS.length + ' tracks / full trip</small>'; all.onclick = () => { state.collection = "all"; renderCollectionChips(); renderPlaylist(); }; w.appendChild(all); ROT.forEach((r) => { const count = SONGS.filter((s) => s.rotation === r.id).length; const b = document.createElement("button"); b.className = "collection-chip" + (state.collection === r.id ? " active" : ""); b.innerHTML = '<strong>' + esc(r.name) + '</strong><small>' + count + ' tracks / ' + esc((r.hours || "on repeat").split(" ")[0]) + '</small>'; b.onclick = () => { state.collection = r.id; renderCollectionChips(); renderPlaylist(); }; w.appendChild(b); }); }
