/* Air Chalo CG-2409 — The Goa Game (front-end, local-first / Supabase-ready) */
(function () {
  "use strict";
  const RADIO = window.GOA_RADIO, TRIP = window.TRIP, DB = window.DB, SB = window.SB;
  ...
  function confetti() {
    ...
    ["🎉", "🌴", "🥥", "✈️", "🎂", "🪩"].forEach((b, i) => {
      const p = document.createElement("i"); p.textContent = b;
      p.style.left = Math.random() * 100 + "%"; p.style.animationDuration = 1.1 + Math.random() + "s";
      p.style.fontSize = 12 + Math.random() * 14 + "px"; layer.appendChild(p); setTimeout(() => p.remove(), 1800);
    });
  }
  function rel(ts) { const s = Math.max(0, Math.floor((Date.now() - new Date(ts).getTime()) / 1000)); if (s < 60) return s + "s ago"; if (s < 3600) return Math.floor(s / 60) + "m ago"; if (s < 86400) return Math.floor(s / 3600) + "h ago"; return Math.floor(s / 86400) + "d ago"; }
  ...
  function loopIndex(rotId) { const list = songsFor(rotId); const total = list.reduce((n, s) => n + (s.dur || 180), 0) || 1; const p = istParts(); let t = (+p.hour * 3600 + +p.minute * 60 + +p.second) % total; for (let i = 0; i < list.length; i++) { const d = list[i].dur || 180; if (t < d) return { song: list[i], offset: t, i }; t -= d; } return { song: list[0], offset: 0, i: 0 }; }
  const fmt = (s) => { s = Math.max(0, Math.floor(s || 0)); return Math.floor(s / 60) + ":" + String(s % 60).padStart(2, "0"); };
  const coverUrl = (id) => `https://i.ytimg.com/vi/${id}/hqdefault.jpg`;
  function paintSong(song, rotId) { current = song; const rot = rotById(rotId || song.rotation); $("#ch-pill").textContent = `CH ${CH_NO[rot.id]} · ${rot.name.toUpperCase()}`; const z = $("#zap"); if (z) { z.classList.remove("go"); void z.offsetWidth; z.classList.add("go"); } $("#cover").src = coverUrl(song.id); $("#cover").onerror = () => $("#cover").src = "icon.svg"; $("#song-title").textContent = song.title; $("#song-artist").textContent = `${song.artist}${song.film ? " · " + song.film : ""} · ${song.year} · ${song.lang}`; $("#yt-link").href = `https://www.youtube.com/watch?v=${song.id}`; $("#ytm-link").href = `https://music.youtube.com/watch?v=${song.id}`; $$(".ch").forEach((b) => b.classList.toggle("on", b.dataset.id === rot.id)); }
  function loadSong(song, seek, autoplay) { paintSong(song, song.rotation); if (!player || !ytReady) return; try { player.loadVideoById({ videoId: song.id, startSeconds: Math.max(0, Math.floor(seek || 0)) }); if (!autoplay) player.pauseVideo(); } catch (e) { player.cueVideoById({ videoId: song.id, startSeconds: Math.max(0, Math.floor(seek || 0)) }); } }
  ...
  function syncPlayBtn() { const b = $("#btn-play"); if (b) b.textContent = playing ? "❚❚ PAUSE" : "▶ ARM AUX"; $("#disc").classList.toggle("spin", playing); }
  ...
  function bootYT() { window.onYouTubeIframeAPIReady = () => { player = new YT.Player("yt-player", { width: 1, height: 1, playerVars: { autoplay: 0, controls: 0, rel: 0, playsinline: 1, modestbranding: 1 }, events: { onReady: () => { ytReady = true; try { player.setVolume(+($("#vol").value || 80)); } catch (e) {} }, onStateChange: (e) => { if (e.data === YT.PlayerState.ENDED) { liveClock ? playLive() : skip(1); } if (e.data === YT.PlayerState.PLAYING) { playing = true; syncPlayBtn(); } if (e.data === YT.PlayerState.PAUSED) { playing = false; syncPlayBtn(); } }, onError: () => skip(1) } }); }; const tag = document.createElement("script"); tag.src = "https://www.youtube.com/iframe_api"; document.head.appendChild(tag); }

  /* ================= HERO / BOARDING ================= */
  function renderSeatSelect() { $("#seat-select").innerHTML = TRIP.friends.map((f, i) => `<button type="button" class="seat-card ${myId === f.id ? "on" : ""}" data-id="${f.id}"><span class="sc-check">✓</span><span class="sc-seat">${DB.SEAT[i]}</span><div class="sc-emoji">${f.emoji}</div><div class="sc-name">${esc(f.name)}</div><div class="sc-nick">${esc(f.nick)}</div></button>`).join(""); $$("#seat-select .seat-card").forEach((b) => b.onclick = () => { myId = b.dataset.id; localStorage.setItem("cg_myid", myId); syncPass(); renderSeatSelect(); }); }
  function syncPass() { const f = TRIP.friends.find((x) => x.id === myId); $("#pass-name").textContent = f ? f.name.toUpperCase() : "PICK A SEAT"; $("#pass-seat").textContent = f ? DB.SEAT[TRIP.friends.findIndex((x) => x.id === myId)] : "—"; const btn = $("#btn-board"); btn.disabled = !f; btn.textContent = f ? "BOARD CG-2409" : "PICK A SEAT FIRST"; $("#my-seat").textContent = f ? DB.SEAT[TRIP.friends.findIndex((x) => x.id === myId)] : "—"; }
  function board() {
    if (!myId) { toast("Pick a seat first"); return; }
    const ov = $("#takeoff"); ov.classList.remove("hidden"); let n = 0; $("#tk-line").textContent = "DOORS CLOSING";
    const iv = setInterval(() => { n++; if (n < 3) $("#tk-line").textContent = ["CABIN LIGHTS", "AUX ARMED"][n - 1]; else { clearInterval(iv); ov.classList.add("hidden"); enterGame(); } }, 900);
  }
  function enterGame() {
  ...
  /* ================= CHAPTER ================= */
  function chapterInfo() { const now = new Date(); const start = new Date(TRIP.start), end = new Date(TRIP.end), bday = new Date(TRIP.birthday); const p = istParts(); const night = (+p.hour >= 21 || +p.hour < 5); if (now < start) return { n: 0, t: "CHAPTER 0 — BEFORE DEPARTURE" }; if (now >= bday) return night ? { n: 3, t: "CHAPTER 3 — THE BIRTHDAY 🎂" } : { n: 2, t: "CHAPTER 2 — WELCOME TO GOA" }; if (now <= end) return night ? { n: 4, t: "CHAPTER 4 — CHAOS MODE 🌙" } : { n: 1, t: "CHAPTER 1 — BOARDING" }; return { n: 5, t: "CHAPTER 5 — FLIGHT HOME" }; }
  function renderChapter() { const c = chapterInfo(); const b = $("#chapter-banner"); if (b) b.textContent = c.t; }
  ...
  function renderDashGame() { const st = DB.ml.active() || { question: { text: "Start a round" } }; const el = $("#dash-game"); el.innerHTML = `<p class="sec-h">🎮 GAME</p><p class="dg-q">Most Likely To…</p><p class="dg-sub">${esc(st.question.text)}</p><a class="dg-link" href="#ml" data-jump="ml">Play →</a>`; }
  ...
  function renderMissions() {
    ...
    $("#mission-list").innerHTML = DB.missions.all().map((m) => {
      ...
      return `<div class="mission ${done ? "done" : ""}">
        ...
        ${done ? `<div class="m-done">✓ Done</div>${ev && ev.evidence_url ? `<img class="m-ev" src="${ev.evidence_url}" alt="evidence"/>` : ""}` : `<div class="m-actions"><button class="m-claim" data-id="${m.id}">COMPLETE MISSION</button><label class="m-up"><input type="file" accept="image/*" data-id="${m.id}" class="m-file"/> UPLOAD EVIDENCE</label></div>`}
      </div>`;
    }).join("");
  ...
  function renderML() {
    ...
    el.innerHTML = `<div class="ml-card">
      ...
      <div class="ml-opts">${TRIP.friends.map((f, i) => `<button class="ml-opt" data-id="${f.id}" ${myVote ? "disabled" : ""}>${f.emoji}<span>${esc(f.name)}</span></button>`).join("")}</div>
  ...
  function runReveal(gid) {
    ...
    let c = 3; box.textContent = c; const iv = setInterval(() => { c--; if (c > 0) { box.textContent = c; } else { clearInterval(iv); mlRevealing = false; const st = DB.ml.active(); const tally = {}; (st ? st.votes : []).forEach((v) => tally[v.selected_id] = (tally[v.selected_id] || 0) + 1); let win = null, max = 0; Object.entries(tally).forEach(([k, v]) => { if (v > max) { max = v; win = k; } }); const w = win ? DB.profiles.get(win) : null; box.innerHTML = `🥁<br><b>${w ? w.avatar + " " + esc(w.name) : "No votes"}</b>${win ? " — " + max + " votes 😂" : ""}`;
      DB.ml.reveal(gid); if (win) { DB.profiles.get(win).votes_recv = (DB.profiles.get(win).votes_recv || 0) + 1; DB.profiles.save(DB.profiles.get(win)); }
  ...
  function renderPassport() { const stamps = DB.passport.stamps(myId); $("#passport-box").innerHTML = `<div class="pass-card"><div class="pc-head">REPUBLIC OF BAD IDEAS</div><div class="pc-sub">Passenger: ${esc(me().name)} · Flight CG-2409</div><div class="pc-stamps">${window.CG_CONTENT.stamps.map((s) => `<div class="pc-stamp ${stamps.includes(s.id) ? "on" : ""}"><span>${s.icon}</span><b>${esc(s.label)}</b></div>`).join("")}</div></div>`; const un = DB.ach.unlocked(myId); $("#ach-grid").innerHTML = DB.ach.all().map((a) => `<div class="ach ${un.includes(a.id) ? "on" : ""}"><span class="ach-ic">${a.icon}</span><b>${esc(a.name)}</b><small>${un.includes(a.id) ? esc(a.description) : "???"}</small></div>`).join(""); }
  function checkAch() { const p = me(); if (p.missions_done >= 3) DB.ach.unlock(myId, "goa_veteran"); if (DB.photos.all().filter((x) => x.user_id === myId).length >= 5) DB.ach.unlock(myId, "photographer"); if (p.chaos_miles >= 1800) { DB.ach.unlock(myId, "main_character"); DB.passport.add(myId, "champion"); } }
  ...
  function setupNav() {
    ...
    ["home", "missions", "ml", "bingo", "vote", "passport", "wrapped", "music"].forEach((id) => { const el = document.getElementById(id); if (el) obs.observe(el); });
  }
  ...
  function tickClocks() {
    ...
    const gl = $("#gate-cd-lbl"); if (gl) gl.textContent = lbl;
    ...
    const prog = (now >= end) ? 1 : (now >= start) ? 0.18 + 0.72 * ((now - start) / (end - start)) : Math.max(0.04, Math.min(0.18, (1 - (start - now) / (60 * 86400000)) * 0.18));
    if ($("#route-fill")) $("#route-fill").style.width = prog * 100 + "%";
    if ($("#route-plane")) $("#route-plane").style.left = prog * 100 + "%";
    if ($("#route-cap")) $("#route-cap").textContent = now > end ? "Arrived. Sand in the overhead bin." : now >= start ? "Over water. Do not look down." : "Taxiing toward 24 September.";
    if ($("#gs")) $("#gs").textContent = String(478 - (+p.second % 6));
  ...
  function init() {
    DB.seed();
    renderSeatSelect(); syncPass();
    $("#btn-board").onclick = board;
    renderChannels(); renderSongs();
    ...
    $("#scrub").onclick = (e) => { if (!player || !ytReady) return; const r = e.currentTarget.getBoundingClientRect(); const pct = Math.min(1, Math.max(0, (e.clientX - r.left) / r.width)); try { player.seekTo((player.getDuration() || 0) * pct, true); liveClock = false; } catch (e) {} };
    const vol = $("#vol"); if (vol) vol.oninput = (e) => { if (player && ytReady) try { player.setVolume(+e.target.value); } catch (e) {} };
    $$("#lang-row button").forEach((b) => b.onclick = () => { langFilter = b.dataset.lang; $$("#lang-row button").forEach((x) => x.classList.toggle("on", x === b)); renderSongs(); });
    $("#btn-ping").onclick = () => { confetti(); toast("Cabin crew notified."); };
    bootYT();
